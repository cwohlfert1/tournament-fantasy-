...
for (const rel of ['pages/SuperAdmin.jsx','pages/TrashTalkTab.jsx']) {
  const f = path.join(__dirname, '..', 'src', rel);
  let src = fs.readFileSync(f, 'utf8'); const before = src;
  src = src.replace(/>\s*(?:×|&times;)\s*<\/button>/g, '><X size={14} /></button>');
