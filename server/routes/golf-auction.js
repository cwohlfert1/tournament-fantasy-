...
async function ensureBudget(leagueId, memberId, auctionBudget, faabBudget) {
  await db.run(`
    INSERT OR IGNORE INTO golf_auction_budgets (id, league_id, member_id, auction_credits_remaining, faab_credits_remaining)
    VALUES (?, ?, ?, ?, ?)
  `, [uuidv4(), leagueId, memberId, auctionBudget, faabBudget]);
}

async function resetFaabIfNeeded(leagueId, memberId, weeklyBudget) {
  const budget = await db.get('SELECT * FROM golf_auction_budgets WHERE league_id = ? AND member_id = ?', [leagueId, memberId]);
  ...
  if (!budget.faab_last_reset || new Date(budget.faab_last_reset) < thisMonday) {
    await db.run('UPDATE golf_auction_budgets SET faab_credits_remaining = ?, faab_last_reset = CURRENT_TIMESTAMP WHERE league_id = ? AND member_id = ?', [weeklyBudget, leagueId, memberId]);
    return weeklyBudget;
  }
...
async function finalizeNomination(session, league) {
  if (!session.current_player_id) return;

  const members = await db.all('SELECT * FROM golf_league_members WHERE golf_league_id = ? ORDER BY draft_order ASC', [league.id]);
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.run('UPDATE golf_auction_budgets SET auction_credits_remaining = MAX(0, auction_credits_remaining - ?) WHERE league_id = ? AND member_id = ?', [winAmount, league.id, winnerId]);

    // Add to winner's roster (skip if already there)
    const alreadyOnRoster = await tx.get('SELECT id FROM golf_rosters WHERE member_id = ? AND player_id = ? AND dropped_at IS NULL', [winnerId, session.current_player_id]);
    if (!alreadyOnRoster) {
      await tx.run('INSERT INTO golf_rosters (id, member_id, player_id) VALUES (?, ?, ?)', [uuidv4(), winnerId, session.current_player_id]);
    }

    // Core player logic for TourneyRun
    if (league.format_type === 'tourneyrun') {
      const winCount = (await tx.get("SELECT COUNT(*) as c FROM golf_auction_bids WHERE league_id = ? AND member_id = ? AND status = 'won' AND bid_type = 'auction'", [league.id, winnerId])).c;
      if (winCount < (league.core_spots || 4)) {
        try { await tx.run('INSERT OR IGNORE INTO golf_core_players (id, member_id, player_id) VALUES (?, ?, ?)', [uuidv4(), winnerId, session.current_player_id]); } catch (_) {}
      }
    }

    // Record the winning bid
    await tx.run("INSERT INTO golf_auction_bids (id, league_id, player_id, member_id, amount, bid_type, status) VALUES (?, ?, ?, ?, ?, 'auction', 'won')", [uuidv4(), league.id, session.current_player_id, winnerId, winAmount]);

    // Check if auction is complete: total wins == members * roster_size
    const totalWins = (await tx.get("SELECT COUNT(*) as c FROM golf_auction_bids WHERE league_id = ? AND status = 'won' AND bid_type = 'auction'", [league.id])).c + 1; // +1 for this win
    const totalNeeded = members.length * (league.roster_size || 8);

    if (totalWins >= totalNeeded) {
      await tx.run("UPDATE golf_auction_sessions SET status='completed', current_player_id=NULL, current_high_bid=1, current_high_bidder_id=NULL, nomination_ends_at=NULL WHERE id=?", [session.id]);
      await tx.run("UPDATE golf_leagues SET draft_status='completed', status='active' WHERE id=?", [league.id]);
    } else {
      ...
      await tx.run("UPDATE golf_auction_sessions SET current_player_id=NULL, current_high_bid=1, current_high_bidder_id=NULL, nomination_ends_at=NULL, current_nomination_member_id=?, nomination_index=? WHERE id=?", [nextNominatorId, nextIndex, session.id]);
    }
  });
}

// ── GET /leagues/:id/auction/state ─────────────────────────────────────────────
router.get('/leagues/:id/auction/state', authMiddleware, async (req, res) => {
  try {
    ...
    `, [req.params.id]);
    for (const m of allMembers) await ensureBudget(req.params.id, m.id, auctionBudget, faabBudget);

    // Get or create session
    let session = await db.get('SELECT * FROM golf_auction_sessions WHERE league_id = ?', [req.params.id]);
    if (!session) {
      const sid = uuidv4();
      await db.run('INSERT INTO golf_auction_sessions (id, league_id, status) VALUES (?, ?, ?)', [sid, req.params.id, 'waiting']);
      session = await db.get('SELECT * FROM golf_auction_sessions WHERE id = ?', [sid]);
    }
    ...
    if (session.status === 'active' && session.current_player_id && session.nomination_ends_at) {
      if (new Date() > new Date(session.nomination_ends_at)) {
        await finalizeNomination(session, league);
        session = await db.get('SELECT * FROM golf_auction_sessions WHERE id = ?', [session.id]);
        // Refresh league in case draft completed
      }
    }
    const freshLeague = await db.get('SELECT * FROM golf_leagues WHERE id = ?', [req.params.id]);

    // Won players
    const wonPlayers = await db.all(`
    ...
    const availablePlayers = (await db.all('SELECT * FROM golf_players WHERE is_active = 1 ORDER BY world_ranking ASC', [])).filter(p => !wonIds.has(p.id));

    // Member budgets
    const budgets = await db.all('SELECT * FROM golf_auction_budgets WHERE league_id = ?', [req.params.id]);
    ...
    const currentPlayer = session.current_player_id
      ? await db.get('SELECT * FROM golf_players WHERE id = ?', [session.current_player_id])
      : null;
...
router.post('/leagues/:id/auction/start', authMiddleware, async (req, res) => {
  try {
    const league = await db.get('SELECT * FROM golf_leagues WHERE id = ?', [req.params.id]);
    if (!league) return res.status(404).json({ error: 'League not found' });
    ...
    await db.transaction(async (tx) => {
      for (let i = 0; i < shuffled.length; i++) {
        ...
        await ensureBudget(req.params.id, shuffled[i].id, auctionBudget, faabBudget);
      }

      const nominationOrder = JSON.stringify(shuffled.map(m => m.id));
      let session = await tx.get('SELECT * FROM golf_auction_sessions WHERE league_id = ?', [req.params.id]);
      if (session) {
        await tx.run("UPDATE golf_auction_sessions SET status='active', current_nomination_member_id=?, nomination_order=?, nomination_index=0, current_player_id=NULL, nomination_ends_at=NULL WHERE id=?", [shuffled[0].id, nominationOrder, session.id]);
      } else {
        await tx.run("INSERT INTO golf_auction_sessions (id, league_id, status, current_nomination_member_id, nomination_order, nomination_index) VALUES (?, ?, 'active', ?, ?, 0)", [uuidv4(), req.params.id, shuffled[0].id, nominationOrder]);
      }
      await tx.run("UPDATE golf_leagues SET draft_status='active', status='drafting' WHERE id=?", [req.params.id]);
    });
...
router.post('/leagues/:id/auction/nominate', authMiddleware, async (req, res) => {
  try {
    ...
    const session = await db.get('SELECT * FROM golf_auction_sessions WHERE league_id = ?', [req.params.id]);
    ...
    const alreadyWon = await db.get("SELECT id FROM golf_auction_bids WHERE league_id = ? AND player_id = ? AND status = 'won' AND bid_type = 'auction'", [req.params.id, player_id]);
    ...
    await db.run('UPDATE golf_auction_sessions SET current_player_id=?, current_high_bid=1, current_high_bidder_id=NULL, nomination_ends_at=? WHERE id=?', [player_id, endsAt, session.id]);
...
router.post('/leagues/:id/auction/bid', authMiddleware, async (req, res) => {
  try {
    ...
    const budget = await db.get('SELECT * FROM golf_auction_budgets WHERE league_id = ? AND member_id = ?', [req.params.id, member.id]);
    ...
    await db.run('UPDATE golf_auction_sessions SET current_high_bid=?, current_high_bidder_id=?, nomination_ends_at=? WHERE id=?', [bid, member.id, newEndsAt, session.id]);
...
router.post('/leagues/:id/faab/bid', authMiddleware, async (req, res) => {
  try {
    ...
    const auctionBudget = league.auction_budget || 1000;
    const faabBudget = league.faab_weekly_budget || 100;
    await ensureBudget(req.params.id, member.id, auctionBudget, faabBudget);
    const faabRemaining = await resetFaabIfNeeded(req.params.id, member.id, faabBudget);
    ...
    // Upsert bid
    const existing = await db.get("SELECT id FROM golf_faab_bids WHERE member_id = ? AND player_id = ? AND status = 'pending'", [member.id, player_id]);
    if (existing) {
      await db.run('UPDATE golf_faab_bids SET bid_amount = ?, drop_player_id = ? WHERE id = ?', [bid, drop_player_id || null, existing.id]);
    ...
    await db.run("INSERT INTO golf_faab_bids (id, golf_league_id, member_id, player_id, drop_player_id, bid_amount, tournament_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')", [uuidv4(), req.params.id, member.id, player_id, drop_player_id || null, bid, activeTournament?.id || null]);
...
router.get('/leagues/:id/faab/bids', authMiddleware, async (req, res) => {
  try {
    const league = await db.get('SELECT * FROM golf_leagues WHERE id = ?', [req.params.id]);
    if (!league) return res.status(404).json({ error: 'League not found' });
    const member = await getMember(req.params.id, req.user.id);
    ...
    const faabBudget = league.faab_weekly_budget || 100;
    await ensureBudget(req.params.id, member.id, league.auction_budget || 1000, faabBudget);
    const faabRemaining = await resetFaabIfNeeded(req.params.id, member.id, faabBudget);
